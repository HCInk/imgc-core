...
torasu::RenderResult* Rtext::render(torasu::RenderInstruction* ri) {
	...
	if (resSettings->getPipeline() == TORASU_STD_PL_VIS) {
		...
		for ( size_t i = 0; i < numChars; i++ ) {
			...
			PointInfo lastPoint;
			PointInfo nextPoint;
			std::vector<PointInfo> drawGroup;
			...
			for (size_t pi = 0; pi < sourcePointCount; pi++) {
				{ // Reading next point
					...
					nextPoint.cord = {static_cast<double>(ftPoint.x)/sacleFactor + cursorX, static_cast<double>(ftPoint.y)/sacleFactor*-1+1};
			...
			if (!drawGroup.empty()) {
				...
				PointInfo* pointData = drawGroup.data();
			...
			cursorX += static_cast<double>(glyph->advance.x)/sacleFactor;
		}
